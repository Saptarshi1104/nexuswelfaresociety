...
<head>
	...
	<?php
	    ...
	    echo '<link rel="stylesheet" href="css/activity_content.css">';
...
<body>
		...
		<div class="nav">
				...
				<div class="nav-menu">
						...
						<li class="list-item list-item-selected">Activity</li>
	...
	<tr>
		<td colspan="2">
			<div class="welcome-head">Activity</div>
		</td>
	</tr>
	<?php
		...
		$activity_query = "SELECT `noticetag`,`day`,`month`,`year`,`text` FROM `activity_table`";
		...
		while($row=mysqli_fetch_array($activity_result))
		{
			...
			echo 			'<div class=news-head>'.$row[0].'</div>';
			echo 			'<div class="news-date">'.$row[1].'-'.$row[2].'-'.$row[3].'</div>';
			echo 			'<div class="news-body">'.$row[4].'</div>';
			echo 		'</div>';
			echo 	'</td>';
			echo '</tr>';
		}
